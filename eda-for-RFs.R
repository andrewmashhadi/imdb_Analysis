...
df_imdb_details <- df_imdb_details %>% mutate(metacriticRatingBinned = cut(metacriticRating, breaks=5),
                                              ...
                                              yearBinned = cut(year, breaks=50),
                                              runtimeBinned = cut(runtime,breaks=10)
                                              )
...
sts <- boxplot.stats(df_imdb_details_movies_only$profit_margin)$stats 
boxplot_output <- ggplot(df_imdb_details_movies_only, aes_string(x="runtimeBinned", y="profit_margin")) +
  geom_boxplot(outlier.shape=NA) +
  coord_cartesian(ylim = c(sts[1]*0.6,sts[5]*1.05)) +
  ggtitle('TBD') 
